...
export const mapWordPressProduct = (wpProduct: any): any => {
  ...
  const parsePrice = (priceString: string | null | undefined): number => {
    ...
    // Also handle cases where price might be formatted like "1,200.00" or "1.200,00"
  ...
  async getCustomerOrders(customerId?: string): Promise<any[]> {
    ...
    return result.customer.orders.edges.map((edge: any) => ({
      ...
      items: edge.node.lineItems.edges.map((item: any) => ({
        productId: item.node.product?.node?.databaseId ? `prod_${item.node.product.node.databaseId}` : 'unknown',
...
export const wordpressAuthService = {
  ...
  async login(username: string, password: string): Promise<any> {
    const result = await fetchGraphQL<any>(LOGIN_MUTATION, {
      username,
      password,
    });
  ...
  async register(data: any): Promise<any> {
    const result = await fetchGraphQL<any>(REGISTER_MUTATION, {
      username: data.username || data.email, // Use email as username if not provided
      email: data.email,
      password: data.password,
    });
    return result?.registerUser || null;
  },
};

// WordPress Checkout Service
export const wordpressCheckoutService = {
  // Checkout
  async checkout(orderData: any): Promise<any> {
    // Map items to lineItems
    const lineItems = orderData.items.map((item: any) => {
      const idStr = item.id || item.productId || '';
      return {
        productId: parseInt(idStr.replace('prod_', '')),
        quantity: item.quantity,
      };
    });

    const input = {
      clientMutationId: `chk_${Date.now()}`,
      billing: orderData.billing,
      shipping: orderData.shipping || orderData.billing, // Fallback to billing if shipping not provided
    ...
    // If user is logged in (has ID), you might iterate to attach customerId, 
    // but WPGraphQL usually handles this via Auth header if provided.

    const result = await fetchGraphQL<any>(CHECKOUT_MUTATION, {
      input,
    });
