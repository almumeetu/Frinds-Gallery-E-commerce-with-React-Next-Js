...
const Logo = () => (
    <div className="text-brand-green font-bold text-2xl tracking-tight text-left">
      Friend's Gallery
    </div>
);

const SocialIcon: React.FC<{ href: string, children: React.ReactNode }> = ({ href, children }) => (
    <a href={href} target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-brand-green transition-colors">
        {children}
    </a>
);
...
export const Footer: React.FC<FooterProps> = ({ navigateTo, navigateToShop }) => {
    return (
        <footer className="bg-white text-slate-600 border-t border-slate-200">
            <ServiceInfo />
            <div className="w-full mx-auto max-w-8xl py-16 px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                    ...
                    <div className="md:col-span-4">
                        <h3 className="text-lg font-semibold text-brand-dark">যোগাযোগ ও নিউজলেটার</h3>
                        ...
                        <h4 className="text-base font-semibold text-brand-dark pt-4 mt-4 border-t">অফার এবং আপডেটের জন্য সাইন আপ করুন</h4>
                        <form className="flex mt-2">
                           <input type="email" placeholder="আপনার ইমেইল দিন" className="w-full rounded-r-none text-sm" />
                           <button className="bg-brand-green text-white px-4 rounded-l-none rounded-r-lg text-sm font-semibold hover:bg-brand-green-dark transition-colors">সাবস্ক্রাইব</button>
                        </form>
                    </div>
                </div>
            </div>
